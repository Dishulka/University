...
def implicit_euler(t_0, t_n, f, h, mode, I):
    m = int((t_n - t_0) / h)

    v = [mode[2]]
    u = [mode[1] * v[0]]

    def implicit_euler_for_v(v_2, v_i, u_i, f, h, I):
    ...
    for i in range(m):
        v.append(scipy.optimize.fsolve(implicit_euler_for_v, v[i], args=(v[i], u[i], f, h, I))[0])
...
def runge_kutta(t_0, t_n, f, h, mode, I):
    m = int((t_n - t_0) / h)
...
if __name__ == '__main__':
    ...
    for i in range(4):
        v_1 = euler(t_0, t_n, f, h, modes[i], I)[0]
        v_2 = implicit_euler(t_0, t_n, f, h, modes[i], I)[0]
        v_3 = runge_kutta(t_0, t_n, f, h, modes[i], I)[0]
